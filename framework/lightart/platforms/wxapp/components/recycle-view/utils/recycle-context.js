...
RecycleContext.prototype._recalculateSizeByProp = function (list, cb) {
  ...
  function sizeReady(rects) {
    rects.forEach((rect, index) => {
      const propValueIndex = needCalcPropIndex[index]
        if(propValueIndex!=undefined){
            propValueMap[propValueIndex].width = rect.width || 0
            propValueMap[propValueIndex].height = rect.height || 0
        }
  ...
  if (calcNewItems.length) {
    ...
    this.page.setData(obj, () => {
      ...
      //   }
      // }).exec();
      wx.createSelectorQuery().selectAll('.u-brand-list-scroll >>> .' + itemSize.queryClass).boundingClientRect((rects) => {
        sizeReady(rects)
      }).exec(function(res) {
          console.log(res);
      })
